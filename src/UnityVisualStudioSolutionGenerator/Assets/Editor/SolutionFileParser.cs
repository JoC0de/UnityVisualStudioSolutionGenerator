...
namespace UnityVisualStudioSolutionGenerator
{
    public static class SolutionFileParser
    {
        ...
        public static IReadOnlyList<ProjectFile> Parse(string content, string solutionDirectoryPath)
        {
            ...
            {
                // unity didn't remove the project-entry generated from us from the solution so we only keep the ones
                // that lies in the solution directory (the one generated by Unity / not changed by us)
                if (!allProjects.Contains(projectFile) && projectFile.FilePath.StartsWith(solutionDirectoryPath, StringComparison.Ordinal))
                {
